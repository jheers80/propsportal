...
export async function POST(request: NextRequest) {
  try {
    ...
    if (delErr) {
      logger.error('Error deleting completion:', delErr);
    ...
    if (updErr) {
      logger.error('Error updating instance status:', updErr);
      return NextResponse.json({ error: updErr.message || updErr }, { status: 500 });
    }

    return NextResponse.json({ success: true });
  } catch (err) {
    logger.error('Error in POST /api/task-instances/uncomplete', err);
